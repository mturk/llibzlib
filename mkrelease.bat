...
rem
rem Usage: mkrelease.bat version arch ["_PDB=1"] ["_STATIC_MSVCRT=1"]
...
rem
rem Create builds
nmake "PREFIX=%BuildDir%\dist\%ReleaseName%" %~3 %~4 %~5 _ASM=1 _STATIC=1 install
nmake "PREFIX=%BuildDir%\dist\%ReleaseName%" %~3 %~4 %~5 _ASM=1 install
rem
